...
Entries up to 2026.03.25 were generated from the git commit history. The fork's
own entries below are maintained by hand, because its add-on versions are
...
- Persist the commanded light zone states across restarts

The commanded toggle states only lived in memory, so restarting the service
lost them and left the zone switches out of sync until each one was tapped
again. They are now written through to the cache.

## [2026.08.29.4] - 2026-08-29 01:25

### 🐛 Bug Fixes

- Clear the light zone switches when the light power goes off

Turning the light off cuts both zones without reporting it, so the switches
went on claiming to be on. The published state is now the light power AND the
last command sent.

## [2026.08.29.3] - 2026-08-29 01:11

### 🐛 Bug Fixes

- Let hass toggle the light zone switches on

Govee reports no state for these toggles, so they sat at unknown, and the hass
frontend answered every tap with turn_off. They now run optimistically.

## [2026.08.29.2] - 2026-08-29 00:27

### 🐛 Bug Fixes

- Address code review findings on the fan entity

## [2026.08.29.1] - 2026-08-28 23:43

First release of the MatrixJson fork.

### 🚀 Features

- Expose Govee fans as Home Assistant fan entities

Devices reporting a fanToggle capability now get a real fan entity with speed
and direction, alongside the switches they had before.

### 🐛 Bug Fixes

- Recover from a stale undoc API login instead of aborting
- Parse new supportEnc and V2 wifi fields from the undoc API

### ⚙️ Miscellaneous

- Point packaging at the MatrixJson fork
- Remove the cosign block so the add-on can build
- Pin the add-on version instead of deriving it in CI
- Fix all clippy warnings across the codebase

## [2026.03.25-ab9deb66] - 2026-03-25 07:36
